...
| REQ-003 | When an allowlisted domain transaction commits the first observation event, the system shall append that event to a bounded durable module outbox in the same transaction without exposing account ownership, credentials, arbitrary paths/URLs, or ungranted private data. | Transaction rollback/commit, privacy, bounds, and direct SQL tests. |
| REQ-004 | When the dispatcher delivers events, the system shall preserve order per exact release/hook/subject partition, apply at-least-once replay receipts, bounded retries/backoff/dead-letter state, saturation backpressure, and an outer deadline without holding a domain database transaction during module execution. | Concurrency, replay, outage, timeout, saturation, and lock-duration tests. |
...
| REQ-007 | When module configuration/state/lifecycle changes or backup/restore occurs, the system shall enforce core-owned namespaces, quotas, CAS revisions, explicit migrations, readiness, expected-state audit, retained rollback, disabled-on-restore, and receipt/outbox reconciliation. | State/lifecycle/migration/backup/isolated-restore tests. |
