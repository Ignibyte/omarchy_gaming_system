---
title: TICKET-064-usurper-level-eleven-dungeon-band
status: closed
ticket_number: 064
type: feature
created: 2026-09-02
closed: 2026-09-02
intake:
pipeline_spec: docs/planning/pipeline/completed/usurper-level-eleven-dungeon-band.spec.md
---

# TICKET-064-usurper-level-eleven-dungeon-band

## Summary

Extend the separate deterministic Usurper provider and signed inert cartridge
with the exact normal Level 11 dungeon band from authenticated v0.20e source,
then prove it through provider replay and provider-backed trusted-QML play.

## Why

Ticket 063 completed Level 10 and retained the repaired one-control,
one-activation live path. Level 11 is the next source-complete ordinary dungeon
slice and fits the existing rules, provider, view, and signed presentation
boundaries without adding dungeon events or platform gameplay authority.
...
- In:
  - v0.20e monster records 100–109 with reviewed base strength and equipment
    flags;
  - source-order `Random(110)` rejection draws, normal selection 101–109, and
    source-derived 60 HP;
  - strict rules/state v16, levels one through eleven, provider action/replay,
...
- Out:
  - Level 12 or higher and composite dungeon event/team/shared-world paths;
...
- Intake: none; the active goal is to continue building the established Usurper game.
- Pipeline spec: [usurper-level-eleven-dungeon-band.spec.md](../../pipeline/completed/usurper-level-eleven-dungeon-band.spec.md)
