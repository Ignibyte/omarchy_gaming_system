---
title: Usurper Level Thirteen Dungeon Band
pipeline_id: 12f2dd10-2c8a-4444-8e76-248d153146ca
status: Phase 5 — Complete PASS
ticket: TICKET-067
ticket_doc: docs/planning/tickets/closed/TICKET-067-usurper-level-thirteen-dungeon-band.md
aar: docs/planning/knowledge/aar/AAR-067-usurper-level-thirteen-dungeon-band.md
created: 2026-09-02
---

# Usurper Level Thirteen Dungeon Band — spec
...
- In:
  - exact v0.20e Level 13 monster records 120–129;
  - normal Level 13 rejection-loop selection and three-times-strength HP;
  - draw-free level switching across levels one through thirteen;
  - rules/state/cartridge v18, bounded `option_m`, deterministic reducers,
...
- Out:
  - Level 14+, dungeon events, quests, finale, shared realm, or new combat
    systems;
...
| # | Decision | Why |
|---|---|---|
| 1 | Port only the normal Level 13 band and preserve event routing as out of scope. | `DUNGEONC.PAS` keeps ordinary monster loading in the event-false branch; events remain an independent unfinished subsystem. |
| 2 | Retain editor record 120 in canonical data but keep normal selection restricted to 121–129 through the original rejection loop. | Record inventory and reachable runtime behavior are separate compatibility facts. |
| 3 | Advance rules, state schema, and cartridge identities together to v18. | Older serialized state and signed action surfaces must not be relabeled as the deeper rules release. |
| 4 | Reuse the generic provider-owned dungeon/combat reducer and add only Level 13 data, bounds, actions, projections, and tests. | OmarchyGS remains transport/rendering authority, never a second game-rules owner. |
| 5 | Add one bounded `option_m` string to the external game view/schema and bind it to exactly one Level 13 action. | The existing A–L fields are occupied on the dungeon screen; widening the data-only game view is smaller and safer than overloading another semantic field or changing platform rendering. |

## Linked artifacts

- Ticket: [TICKET-067](../../tickets/closed/TICKET-067-usurper-level-thirteen-dungeon-band.md)
...
- Intake: continuing goal after completing the Level 12 normal dungeon band.
...
| 3 Implement | External rules/data/provider/cartridge Level 13 slice | compile and self-review |
