---
title: Usurper Level-Eight Dungeon Band
pipeline_id: ae7ba576-c2df-471d-a162-e4a3bf30395e
status: Phase 5 — Complete PASS
ticket: TICKET-060
ticket_doc: docs/planning/tickets/closed/TICKET-060-usurper-level-eight-dungeon-band.md
aar: docs/planning/knowledge/aar/AAR-060-usurper-level-eight-dungeon-band.md
created: 2026-09-02
---

# Usurper Level-Eight Dungeon Band — spec

## Intent

Ship the next source-complete normal dungeon band in the separate Usurper Rust
provider: exact level-eight data and legacy selection, deterministic combat
...
- Out:
  - level nine or higher and the composite dungeon event/team/shared-world paths;
...
| # | Decision | Why |
|---|---|---|
| 1 | Port the canonical v0.20e Level 8 records at indices 70–79 exactly. | Preserves the chosen publisher-linked baseline rather than inventing balance data. |
| 2 | Retain record 70 while normal selection accepts only 71–79. | The original rejection loop makes the boundary record unreachable but still part of the source table. |
| 3 | Preserve every rejected `Random(80)` draw in deterministic state. | Discarded source RNG work changes all subsequent replay-visible outcomes. |
| 4 | Advance state, rules, and cartridge identity to v13 without migrating v12 state. | Avoids silently interpreting older persisted JSON under broader rules. |
...
- Ticket: `docs/planning/tickets/closed/TICKET-060-usurper-level-eight-dungeon-band.md`
