---
title: Usurper Level Sixteen Dungeon Band
pipeline_id: 805147ae-42f4-4f23-a1ee-fa9c8bd39498
status: Phase 5 — Complete PASS
ticket: TICKET-070
ticket_doc: docs/planning/tickets/closed/TICKET-070-usurper-level-sixteen-dungeon-band.md
aar: docs/planning/knowledge/aar/AAR-070-usurper-level-sixteen-dungeon-band.md
created: 2026-09-03
---

# Usurper Level Sixteen Dungeon Band — spec
...
- In:
  - exact v0.20e Level 16 monster records 150–159;
  - normal Level 16 rejection-loop selection and three-times-strength HP;
  - draw-free level switching across levels one through sixteen;
  - rules/state/cartridge v21, bounded sixteenth-choice projection,
...
- Out:
  - Level 17+, dungeon events, quests, finale, shared realm, or new combat
    systems;
...
| # | Decision | Why |
|---|---|---|
| 1 | Port only the normal Level 16 band and preserve event routing as out of scope. | `DUNGEONC.PAS` keeps ordinary monster loading in the event-false branch; events remain an independent unfinished subsystem. |
| 2 | Retain editor record 150 in canonical data but keep normal selection restricted to 151–159 through the original rejection loop. | Record inventory and reachable runtime behavior are separate compatibility facts. |
| 3 | Advance rules, state schema, and cartridge identities together to v21. | Older serialized state and signed action surfaces must not be relabeled as the deeper rules release. |
| 4 | Reuse the generic provider-owned dungeon/combat reducer and add only Level 16 data, bounds, actions, projections, and tests. | OmarchyGS remains transport/rendering authority, never a second game-rules owner. |
| 5 | Add one bounded sixteenth-choice string to the external game view/schema and bind it to exactly one Level 16 action. | The existing A–O fields are occupied on the dungeon screen; widening the data-only game view is smaller and safer than overloading another semantic field or changing platform rendering. |

## Linked artifacts

- Ticket: [TICKET-070](../../tickets/closed/TICKET-070-usurper-level-sixteen-dungeon-band.md)
...
- Intake: continuing goal after completing the Level 15 normal dungeon band.
...
| 3 Implement | External rules/data/provider/cartridge Level 16 slice | compile and self-review |
