---
title: Usurper Level-Six Dungeon Band
pipeline_id: 57bc1563-b5c5-42ef-824f-612c500966e5
status: Phase 5 — Complete PASS
ticket: TICKET-058
ticket_doc: docs/planning/tickets/closed/TICKET-058-usurper-level-six-dungeon-band.md
aar: docs/planning/knowledge/aar/AAR-058-usurper-level-six-dungeon-band.md
created: 2026-08-31
---

# Usurper Level-Six Dungeon Band — spec

## Intent

Ship the next source-complete normal dungeon band in the separate Usurper Rust
provider: exact level-six data and legacy selection, deterministic combat
composition, a signed inert control, and a visible trusted-QML preview.

## Scope

- In:
  - v0.20e monster records 50–59 with reviewed strength and equipment flags;
  - source-order `Random(60)` rejection draws, normal selection 51–59, and
    source-derived 48 HP;
  - strict rules/state v11, levels one through six, provider action/replay,
    signed cartridge, fixtures, provenance, tests, documentation, and preview.
- Out:
  - level seven or higher and the composite dungeon event/team/shared-world paths;
...
| # | Decision | Why |
|---|---|---|
| 1 | Port the canonical v0.20e Level 6 records at indices 50–59 exactly. | Preserves the chosen publisher-linked baseline rather than inventing balance data. |
| 2 | Retain record 50 while normal selection accepts only 51–59. | The original rejection loop makes the boundary record unreachable but still part of the source table. |
| 3 | Preserve every rejected `Random(60)` draw in deterministic state. | Discarded source RNG work changes all subsequent replay-visible outcomes. |
| 4 | Advance state, rules, and cartridge identity to v11 without migrating v10 state. | Avoids silently interpreting older persisted JSON under broader rules. |
...
- Ticket: `docs/planning/tickets/closed/TICKET-058-usurper-level-six-dungeon-band.md`
