---
title: Usurper Level Twelve Dungeon Band
pipeline_id: 4ec0cc25-d550-41c3-87a3-57b934c3c8d6
status: Phase 5 — Complete PASS
ticket: TICKET-066
ticket_doc: docs/planning/tickets/closed/TICKET-066-usurper-level-twelve-dungeon-band.md
aar: docs/planning/knowledge/aar/AAR-066-usurper-level-twelve-dungeon-band.md
created: 2026-09-02
---

# Usurper Level Twelve Dungeon Band — spec
...
- In:
  - exact v0.20e Level 12 monster records 110–119;
  - normal Level 12 rejection-loop selection and three-times-strength HP;
  - draw-free level switching across levels one through twelve;
  - rules/state/cartridge v17, bounded `option_l`, deterministic reducers,
...
- Out:
  - Level 13+, dungeon events, quests, finale, shared realm, or new combat
    systems;
...
| # | Decision | Why |
|---|---|---|
| 1 | Port only the normal Level 12 band and preserve event routing as out of scope. | `DUNGEONC.PAS` keeps ordinary monster loading in the event-false branch; events remain an independent unfinished subsystem. |
| 2 | Retain editor record 110 in canonical data but keep normal selection restricted to 111–119 through the original rejection loop. | Record inventory and reachable runtime behavior are separate compatibility facts. |
| 3 | Advance rules, state schema, and cartridge identities together to v17. | Older serialized state and signed action surfaces must not be relabeled as the deeper rules release. |
| 4 | Reuse the generic provider-owned dungeon/combat reducer and add only Level 12 data, bounds, actions, projections, and tests. | OmarchyGS remains transport/rendering authority, never a second game-rules owner. |
| 5 | Add one bounded `option_l` string to the external game view/schema and bind it to exactly one Level 12 action. | The existing A–K fields are all occupied on the dungeon screen; widening the data-only game view is smaller and safer than overloading another semantic field or changing platform rendering. |

## Linked artifacts

- Ticket: [TICKET-066](../../tickets/closed/TICKET-066-usurper-level-twelve-dungeon-band.md)
...
- Intake: continuing goal after closing the local-play control regression.
...
| 3 Implement | External rules/data/provider/cartridge Level 12 slice | compile and self-review |
