...
async fn run() -> Result<(), OperatorError> {
    ...
    let action = arguments.next().ok_or(OperatorError::InvalidInput)?;
    let database_url = env::var("DATABASE_URL").map_err(|_| OperatorError::InvalidInput)?;
    ...
    serde_json::to_writer(std::io::stdout(), &output).map_err(|_| OperatorError::Internal)?;
    writeln!(std::io::stdout()).map_err(|_| OperatorError::Internal)?;
    Ok(())
}
